/* ============================================================================
   Adaptador Vercel -> Cloudflare Pages Functions
   ----------------------------------------------------------------------------
   ...
   Cloudflare Pages Functions usa la Fetch API: `export function onRequest
   ({ request, env }) { ... return new Response(...) }`. En vez de reescribir
   cada endpoint (arriesgando romper la lógica de precios, el lock atómico de
   reservas o el escapado contra XSS, todo cosa ya probada), este archivo
   traduce de un mundo al otro. Los 8 archivos de `functions/api/` son sólo
   una línea que importa el handler original y lo envuelve con `wrap()`.
