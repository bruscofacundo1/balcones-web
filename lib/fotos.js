...
/**
 * Vercel dejó de inyectar `BLOB_READ_WRITE_TOKEN` al conectar un store: ahora
 * usa OIDC por defecto (más seguro, token de corta vida) y pone en su lugar
 * `VERCEL_OIDC_TOKEN` + `BLOB_STORE_ID`. El SDK de `@vercel/blob` entiende las
 * dos formas — `put()`/`del()` no necesitan tocarse — pero este chequeo
 * buscaba sólo la vieja, así que el panel decía "no se puede subir" con las
 * dos variables nuevas puestas y andando.
 */
