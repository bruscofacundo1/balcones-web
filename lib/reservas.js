...
/** Sólo dice si la variable está presente (nunca su valor). Para diagnosticar. */
function diagnostico() {
  // Lista TODAS las variables de entorno que se parecen a "database" o "url",
  // con su nombre entre corchetes para que un espacio invisible al final se
  // note en el JSON (sale como "DATABASE_URL " con el espacio antes del
  // cierre de comillas).
  const parecidas = Object.keys(process.env)
    .filter(k => /database|neon|postgres/i.test(k))
    .map(k => `[${k}]`);
  return { DATABASE_URL: Boolean(process.env.DATABASE_URL), variablesParecidas: parecidas };
}

module.exports = { hayBaseDatos, nochesPagadas, marcarPagada, pagoYaProcesado, marcarPagoProcesado, diagnostico };
